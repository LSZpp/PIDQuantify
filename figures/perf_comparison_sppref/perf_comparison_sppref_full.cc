...
int main(){
    ...
    auto make_perf = [&](const std::string &p1, const std::string &p2, const std::string &tag){
        for (const double cut : {0., 5.}){
            ...
            curves.add_perf("24b1",    "up", "24b1 MAGup",        dir);
            curves.add_perf("24b8",    "up", "24b8 MAGup",        dir);
            const std::string name = tag + (cut == 0. ? "_gt0" : "_gt5");
            curves.create_figures(name, .7, 1.1, &colours);
            curves.export_canvases();
        }
    };

    make_perf("P",  "K",  "perf_P_sppref_b1_b8_magup_DLLp_DLLK");
    make_perf("K",  "Pi", "perf_K_sppref_b1_b8_magup_DLLK");
    make_perf("Pi", "K",  "perf_Pi_sppref_b1_b8_magup_DLLKneg");
