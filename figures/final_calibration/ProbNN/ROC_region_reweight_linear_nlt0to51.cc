...
// Copy of ROC_region_reweight_linear.cc for the LOW track-multiplicity region
// (nLongTracks [0,51]); p and eta unchanged from the base script.
...
//   p[22000,55000] MeV, eta[2.73,3.54], nLongTracks[0,51]
// mis-ID y-axis floored at 1e-5.
//
// Compile + run with ./compile_and_run.sh ROC_region_reweight_linear_nlt0to51.cc
...
int main(){
    ...
    const QHistogramSource new_dll = QHistogramSource::finals_3d(DIR, "newL0", 0., 51.);
    const QHistogramSource lc_dll  = QHistogramSource::finals_3d(DIR, "Lc",    0., 51.);
    const QHistogramSource k_dll   = QHistogramSource::finals_3d(DIR, "K",     0., 51.);
    const QHistogramSource new_pnn = QHistogramSource::finals_3d(DIR, "newL0", 0., 51., CS::ProbNNLinear);
    const QHistogramSource lc_pnn  = QHistogramSource::finals_3d(DIR, "Lc",    0., 51., CS::ProbNNLinear);
    const QHistogramSource k_pnn   = QHistogramSource::finals_3d(DIR, "K",     0., 51., CS::ProbNNLinear);
    ...
    curves.create_figure("region_reweight_dll_probnn_linear_2025_c3_nlt0to51", &colours, &markers,
                         {0.95, 1.}, {1.e-5, 1.});
