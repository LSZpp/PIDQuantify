// DLL vs ProbNN p->K ROC in the controlled kinematic region, for 2025 c3 (up),
// the only dataset with ProbNN 3D effhists. One figure, four curves:
//   DLL    : Updated L0 (kRed+1) and Lc (kBlue+2), open square  (25)
//   ProbNN : Updated L0 (kRed-1) and Lc (kBlue-1), open circle  (24)
// (open markers throughout; square = DLL, circle = ProbNN).
//
// DLL scan: DLLp-DLLK in -50..50 step 0.5.  ProbNN scan: PROBNN_P>exp(log),
// log in -15..0 step 0.05 (301 points). Region (enclosed bins):
//   p[22000,26000] MeV, eta[2.45,2.59], nLongTracks[83,117].
// mis-ID y-axis floored at 1e-5.
//
// Compile + run with ./compile_and_run.sh ROC_region_probnn.cc
...
int main(){
    ...
    // ProbNN 3D sources.
    const QHistogramSource new_pnn = QHistogramSource::finals_3d(DIR, "newL0", 83., 117., CS::ProbNN);
    const QHistogramSource lc_pnn  = QHistogramSource::finals_3d(DIR, "Lc",    83., 117., CS::ProbNN);
    const QHistogramSource k_pnn   = QHistogramSource::finals_3d(DIR, "K",     83., 117., CS::ProbNN);
    ...
    curves.add_curve("25c3", "up", PNN_NEW, -15., 0., .05, new_pnn, k_pnn);
    curves.add_curve("25c3", "up", PNN_LC,  -15., 0., .05, lc_pnn,  k_pnn);
    curves.create_figure("region_dll_probnn_2025_c3", &colours, &markers, {0.7, 1.005}, {1.e-5, 1.});
