// Per-dataset 2D proton-ID efficiency maps (P vs ETA) at DLLp-DLLK>5, one
// subdirectory per 2024/2025 dataset (<tag>/). Same formatting as the combined
// macro: percent text (1 dp), GeV/c momentum axis, 256 contours, no title,
// 1200x800. Efficiency colour scale fixed 50-100 in kRainbow (sub-50 bins show
// the floor colour and keep their text); diffs in kThermometer with fixed
// ranges (newL0-oldL0: +/-1.5 ; Lc-newL0: +/-10). Bins with <100 events killed.
//
// Run:  root -l -b -q perf_finals_2d.C

#include <vector>
#include <cmath>

const TString DIR     = "/data/lhcb/users/lins/u3_PIDQuantify/finals_hists/2d";
const TString OUT     = "/home/lins/u3_PIDQuantify/figures/final_calibration/2d_differences";
...
// Efficiency (in %) for one (sample dir, dataset, polarity); fills tot. nullptr if missing.
TH2D *load_one(const char *setdir, const TString &ds, const TString &pol, const TString &uname, TH2D **totOut){
    TString path=Form("%s/%s/effhists-%s-%s-P-%s-P.ETA.root",DIR.Data(),setdir,ds.Data(),pol.Data(),CUT.Data());
    TFile *f=TFile::Open(path);
    if (!f||f->IsZombie()){ if(f) delete f; *totOut=nullptr; return nullptr; }
    TH2D *t=(TH2D*)f->Get("total"), *p=(TH2D*)f->Get(Form("passing_%s",CUT.Data()));
    if (!t||!p){ f->Close(); delete f; *totOut=nullptr; return nullptr; }
    TH2D *eff=(TH2D*)p->Clone(uname+"_eff"); eff->SetDirectory(0);
    eff->Divide(p,t,1.,1.,"B"); eff->Scale(100.0);
    TH2D *tot=(TH2D*)t->Clone(uname+"_tot"); tot->SetDirectory(0);
    f->Close(); delete f;
...
// Absolute efficiency: colour fixed 50-100 (kRainbow). Two layers: colour layer
// with sub-50 bins clamped to the floor (so they show the minimum colour instead
// of being whited by ROOT) + text layer with the true values. Killed bins
// (tot<MINEVT) stay exact 0 and are whited by COL1.
...
void perf_finals_2d(){
    ...
    for (auto &d : kData){
        TString sub=OUT+"/"+d.tag; gSystem->mkdir(sub,kTRUE);
        TH2D *oT,*nT,*lT;
        TH2D *oE=load_one("oldL0", d.oldL0, d.pol, d.tag+"_o", &oT);
        TH2D *nE=load_one("newL0", d.newL0, d.pol, d.tag+"_n", &nT);
        TH2D *lE=load_one("Lc",    d.lc,    d.pol, d.tag+"_l", &lT);
        printf("%-6s : old=%s new=%s Lc=%s\n", d.tag.Data(), oE?"ok":"--", nE?"ok":"--", lE?"ok":"--");
        draw_eff (oE,oT,sub,"eff_oldL0",d.tag+"o");
        draw_eff (nE,nT,sub,"eff_newL0",d.tag+"n");
        draw_eff (lE,lT,sub,"eff_Lc",   d.tag+"l");
        draw_diff(nE,oE,nT,oT,sub,"diff_newL0_minus_oldL0",d.tag+"dno", -1.5, +1.5);
        draw_diff(lE,nE,lT,nT,sub,"diff_Lc_minus_newL0",   d.tag+"dln", -10., +10.);
    }
    printf("done.\n");
}
